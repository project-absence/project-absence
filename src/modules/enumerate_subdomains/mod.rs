...
pub struct ModuleEnumerateSubdomains {
    config: config::EnumerateSubdomainsConfig,
    match_status: Vec<usize>,
}

impl ModuleEnumerateSubdomains {
    pub fn new(config: config::EnumerateSubdomainsConfig) -> Self {
...
impl Module for ModuleEnumerateSubdomains {
    ...
    fn execute(&self, session: &Session, context: Context) -> Result<(), String> {
        ...
        for line in lines.map_while(Result::ok) {
            ...
            if session.get_state().has_discovered_domain(uri.clone()) {
                continue;
            }
                ...
                .send()
            {
                ...
                if match_status.contains(&(status_code.as_u16() as usize)) {
                    logger::println(
                        self.name(),
                        format!("Discovered '{}' ({}) as a new subdomain", uri, status_code),
                    );
